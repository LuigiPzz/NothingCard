...
fun PixelDissolveContainer(
    ...
    val particles = remember(isDissolving || isResolving) {
        if (isDissolving || isResolving) List(800) { NothingParticle() }
        else emptyList()
    }
                ...
                particles.forEach { particle ->
                    ...
                    if (particleAlpha > 0f) {
                        val color = when (particle.colorType) {
                            0 -> Color.White
                            1 -> Color.LightGray
                            else -> Color(0xFFFF0000) // Iconic Nothing Red
                        }
...
private class NothingParticle {
    ...
    private val speed = Random.nextFloat() * 2.5f + 0.5f
